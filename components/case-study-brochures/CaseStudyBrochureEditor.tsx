    ...
    <header className="mx-auto grid max-w-[86rem] gap-10 px-6 pb-16 pt-10 sm:px-10 sm:pt-16 lg:grid-cols-[minmax(0,1fr)_20rem] lg:px-16 lg:pb-24">
      ...
      <div className="self-end border-t border-brief-line pt-5">
        <strong className="block text-5xl font-black tracking-[-0.06em]">
          {draftedSections}/{allSectionCount}
        </strong>
...
function CollectionFraming({
  collection,
  onChange,
}: {
  collection: BrochureDocument['collection']
  onChange: (patch: Partial<BrochureDocument['collection']>) => void
}) {
  return (
      ...
      <details>
        ...
        <div className="grid gap-6 pb-8 pt-5 lg:grid-cols-2">
          <TextArea
            ...
            value={collection.throughline}
            onChange={value => onChange({ throughline: value })}
            rows={5}
          />
          <TextArea
...
function StudyPositioning({
  study,
  onChange,
}: {
  study: BrochureStudy
  onChange: (patch: Partial<BrochureStudy>) => void
}) {
  return (
    <section className="mx-auto grid max-w-[86rem] gap-10 px-6 pb-8 pt-12 sm:px-10 lg:grid-cols-[18rem_minmax(0,1fr)] lg:gap-20 lg:px-16">
      ...
      <div className="grid min-w-0 gap-6">
        <TextField
          ...
          value={study.recommendedHeadline}
          onChange={value => onChange({ recommendedHeadline: value })}
        />
        <StringListEditor
          label="Alternate headlines"
          values={study.alternateHeadlines}
          onChange={alternateHeadlines => onChange({ alternateHeadlines })}
          addLabel="Add headline"
        />
        <TextArea
          label="Standfirst"
          value={study.standfirst}
          onChange={value => onChange({ standfirst: value })}
          rows={4}
        />
        <TextArea
          label="Narrative spine"
          help="This is editorial guidance, not necessarily published copy."
          value={study.narrativeSpine}
          onChange={value => onChange({ narrativeSpine: value })}
          rows={5}
        />
        <StringListEditor
          label="Context strip"
          values={study.context}
...
function BrochureSections({
  ...
  onChange: (sectionId: string, patch: Partial<BrochureSection>) => void
}) {
  return (
    ...
    <section className="mx-auto grid max-w-[86rem] gap-10 px-6 py-16 sm:px-10 lg:grid-cols-[18rem_minmax(0,1fr)] lg:gap-20 lg:px-16">
      ...
      <div className="grid gap-8">
        <StringListEditor
          ...
          values={study.askMeAbout}
          onChange={askMeAbout => onChange({ askMeAbout })}
        ...
        <StringListEditor
          ...
          values={study.leaveOff}
          onChange={leaveOff => onChange({ leaveOff })}
          addLabel="Add excluded detail"
        />
        <StringListEditor
          label="Resolve before publishing"
          values={study.factChecks}
          onChange={factChecks => onChange({ factChecks })}
        ...
        {study.assets.map((item, index) => (
          ...
          >
            ...
            <SelectField
              ...
              onChange={value =>
                onChange({
                  assets: study.assets.map(assetItem =>
