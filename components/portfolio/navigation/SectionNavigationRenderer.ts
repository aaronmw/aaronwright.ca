  ...
  render(state: NavigationRenderState) {
    ...
    ;(['left', 'right'] as const).forEach(side => {
      ...
      if (ring) {
        ...
        this.setAttribute(
          ring,
          'data-navigation-coordinate',
          String(state.coordinate),
        )
      }
  ...
  private renderAffordances(state: NavigationRenderState) {
    ...
    ;(['left', 'right'] as const).forEach(side => {
      ...
      context.items.forEach((item, itemIndex) => {
        ...
        const { arrowOpacity, dotOpacity } = this.getAffordanceLayers(itemIndex)
        const rotation = item.pending ? 0 : side === 'left' ? 90 : -90
        const concealed =
          context.modalPresentationActive && itemIndex !== context.activeIndex
        const dimmed =
